...
# properties Package Documentation

This package holds the canonical names of the normalized `properties` fields,
the property contract between the service that discovers a resource and the one
that builds the graph from it. Using the constants instead of string literals
makes a divergent key (`parentID` vs `parentId`, `vpcId` vs `networkId`) a
compile error rather than a silently broken join.

`IsCanonical` is the gate a producer applies to a flat property map: a canonical
key is emitted, everything else - raw provider fields and values that are not
canonical yet - is dropped. A field crosses into the contract simply by being
given a constant here.

Values derived later by the exposure engine are deliberately absent, so a
producer cannot emit one as part of the contract.

## Example Usage

```go
for key, value := range fields {
	if properties.IsCanonical(key) {
		resource.Properties[key] = value
	}
}

resource.Properties[properties.ParentID] = vpcID
```
...
```go
import "github.com/greenbone-hive/opensight-golang-libraries/pkg/assets/properties"
```

Package properties holds the canonical names of the normalized \`properties\` fields \(the topology node\-property contract\): the keys discovery emits and the topology graph\-build reads. Using these constants instead of string literals makes a misspelled or divergent key \(parentID vs parentId, vpcId vs networkId, account\_id vs accountId\) a compile error instead of a silently broken graph join. Raw provider\-specific keys are NOT here: they are not the contract, and a key earns a constant only once both sides agree on it.

## Index

- [Constants](<#constants>)
- [Variables](<#variables>)
- [func IsCanonical\(key string\) bool](<#IsCanonical>)
...
<a name="ParentID"></a>Tenancy / Governance.
...
<a name="SubnetIDs"></a>Gateways: NAT / CloudRouter / Bastion / ManagedFirewall / FirewallPolicy.
...
<a name="IPAddress"></a>IP addressing \+ DNS : PublicIpAddress / DnsZone / DnsRecord.
...
<a name="Scheme"></a>Ingress: the internet \-\> workload funnel. LoadBalancer / ApplicationGateway reuse networkId / subnetIds / publicIpIds / securityGroupIds; LbListener / Cdn reuse the shared object\-array serialization \(jsonArrayKeys on the consumer side\) for their action/rule arrays.
...
<a name="PublicNetworkAccess"></a>Data\-plane gates \(leaf/compute assets: DB, Storage, Cache, k8s\). These are producer\-emitted normalized fields that reachability reads as a SECOND ingress gate, independent of any NSG: a public endpoint is only reachable when its own allow\-list admits the peer. They are registered here AHEAD of the clients that emit them so a client can emit them the moment it needs to; an unregistered key is dropped by discovery and would be invisible to reachability \(a false\-negative public exposure\).
...
## func [IsCanonical](<https://github.com/greenbone-hive/opensight-golang-libraries/blob/main/pkg/assets/properties/properties.go#L914>)
