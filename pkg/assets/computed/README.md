...
# computed Package Documentation

This package holds the canonical names of the normalized `computed` bucket
fields, the property contract between the service that discovers a resource and
the one that builds the graph from it. Using the constants instead of string
literals makes a divergent key (`parentID` vs `parentId`, `vpcId` vs `networkId`)
a compile error rather than a silently broken join.

`IsCanonical` is what splits a flat property map into the two buckets: a
canonical key belongs in `Computed`, everything else - raw provider fields and
values that are not canonical yet - belongs in `Properties`. A field crosses into
`Computed` simply by being given a constant here.

Values derived later by the exposure engine are deliberately absent, so a
producer that emits one has it routed to `Properties` instead of passing as part
of the contract.
...
for key, value := range properties {
	if computed.IsCanonical(key) {
		resource.Computed[key] = value
	} else {
		resource.Properties[key] = value
	}
}

resource.Computed[computed.ParentID] = vpcID
```
...
```go
import "github.com/greenbone-hive/opensight-golang-libraries/pkg/assets/computed"
```

Package computed holds the canonical names of the normalized \`computed\` bucket fields \(the topology node\-property contract\): the keys discovery emits and the topology graph\-build reads. Using these constants instead of string literals makes a misspelled or divergent key \(parentID vs parentId, vpcId vs networkId, account\_id vs accountId\) a compile error instead of a silently broken graph join. Raw provider\-specific keys are NOT here: they are not the contract, and a key earns a constant only once both sides agree on it.
...
## func [IsCanonical](<https://github.com/greenbone-hive/opensight-golang-libraries/blob/main/pkg/assets/computed/computed.go#L914>)
