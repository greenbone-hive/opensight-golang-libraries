...
// Two target partitions under one connector have independent ordering keys:
...
func TestScanOrderingContract(t *testing.T) {
	e := scan("111111111111", 6, CoverageComplete)
	e.EntityID = "conn-1" // connector-wide stream: two targets would share it
	...
	for _, missing := range []func(*ScanCompleted){
		func(e *ScanCompleted) { e.ConnectorID = "" },
...
func TestScopeRetirementReasonGate(t *testing.T) {
	p := Provenance{ConnectorID: "conn-1", TargetScopeID: "111111111111", Provider: "aws"}
